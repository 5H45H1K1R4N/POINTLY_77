    ...
    suspend fun <T : Any> getDocument(collection: String, documentId: String, clazz: Class<T>): T? {
        ...
        return if (snapshot.exists()) {
            try {
                ...
                Log.e("FirestoreRepository", "Error deserializing getDocument from $collection/$documentId to ${clazz.simpleName}", e)
                null
            }
        } else null
    }
            ...
            .addSnapshotListener { snapshot, error ->
                ...
                if (snapshot != null && snapshot.exists()) {
                    try {
                        ...
                        Log.e("FirestoreRepository", "Error deserializing listenDocument from $collection/$documentId to ${clazz.simpleName}", e)
                        trySend(null)
                    }
                } else {
                    trySend(null)
                ...
                if (snapshot != null) {
                    val list = snapshot.documents.mapNotNull { doc ->
                        try {
                            ...
                            Log.e("FirestoreRepository", "Error deserializing listenCollection item from $collection to ${clazz.simpleName}", e)
                            null
                        }
                    }
                    trySend(list)
